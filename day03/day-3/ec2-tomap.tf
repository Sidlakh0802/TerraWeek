...
resource "aws_instance" "my_ec2_instance" {
  for_each = tomap({ #this will create instances based on the map provided in the for_each argument(Meta argument)
    terraweek1 = "t3.micro",
    terraweek2 = "t3.micro",
  ...
  user_data              = file("${path.module}/nginix-setup.sh")  #this will run the script on the instance when it is created
